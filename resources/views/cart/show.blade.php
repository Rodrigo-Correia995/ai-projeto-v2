@php
    $currentInterval = \App\Models\ShippingCost::where('min_value_threshold', '<=', $cart['subtotal'])
                           ->where('max_value_threshold', '>=', $cart['subtotal'])
                           ->first();
@endphp
            ...
            <div class="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="mt-4 bg-gray-50 " >
                        <p>Shipping Cost: {{ number_format($cart['shipping'], 2) }}€</p>
                            @if($currentInterval)
                        <p class="text-sm text-gray-600">
                            Applies to orders between 
                            {{ number_format($currentInterval->min_value_threshold, 2) }}€ 
                            and 
                            {{ number_format($currentInterval->max_value_threshold, 2) }}€
                        </p>
                            @endif
                    </div>
                        ...
                        @if(auth()->user())
                            <form action="{{ route('cart.confirm') }}" method="POST">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    ...
                                    <div>
                                        <label for="delivery_address" class="block mb-1">Delivery Address</label>
                                        <input type="text" name="delivery_address" id="delivery_address" 
                                               value="{{ old('delivery_address', auth()->user()->default_delivery_address ?? '') }}" 
                                               class="w-full px-3 py-2 border rounded">
                                    </div>
                                </div>
